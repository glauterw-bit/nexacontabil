...
export class OneDriveService {
  ...
  private async _scanDrives(token: string, drives: Array<{ id: string; site: string; name: string }>, inicio: number, budget: number, maxDrives?: number) {
    ...
    for (const d of drives.slice(0, maxDrives ?? drives.length)) {
      ...
      let dguard = 0, d429 = 0, novoDelta: string | null = null, vistosDrive = 0;
      while (durl && dguard++ < 400) {
        if (Date.now() - inicio > budget) { durl = null; break; }
        const res: any = await fetch(durl, { headers: { Authorization: `Bearer ${token}` } });
        if ((res.status === 429 || res.status === 503) && d429 < 6) { d429++; await new Promise((r) => setTimeout(r, Math.min(30, 2 ** d429) * 1000)); dguard--; continue; }
        if (res.status === 410) { durl = `${GRAPH_BASE}/drives/${d.id}/root/delta?$select=name,file,parentReference,lastModifiedDateTime&$top=500`; continue; } // resync
        if (!res.ok) break;
        const json: any = await res.json();
      ...
      if (novoDelta) {
        await this.prisma.driveDeltaState.upsert({
          where: { driveId: d.id },
          create: { driveId: d.id, siteName: d.site, driveName: d.name, deltaLink: novoDelta, arquivos: vistosDrive },
          update: { deltaLink: novoDelta, arquivos: vistosDrive, siteName: d.site, driveName: d.name, lastSyncAt: new Date() },
        }).catch(() => undefined);
      }
      drivesVarridos++;
    }
   ...
   */
  async lerPlanilhaXlsx(connectionId: string, driveId: string, itemId: string, opts?: { maxRows?: number; maxMB?: number; aba?: string }): Promise<{ linhas: string[][]; aba: string; abas: string[] }> {
    const maxMB = opts?.maxMB ?? 30, maxRows = opts?.maxRows ?? 5000;
    const token = await this.getValidToken(connectionId);
    const res: any = await fetch(`${GRAPH_BASE}/drives/${driveId}/items/${itemId}/content`, { headers: { Authorization: `Bearer ${token}` }, redirect: 'follow' });
    if (!res.ok) return { linhas: [], aba: '', abas: [] };
    const buf = Buffer.from(await res.arrayBuffer());
    if (buf.length > maxMB * 1024 * 1024) return { linhas: [], aba: '', abas: [] };
    // SheetJS lê .xls (BIFF) e .xlsx; cellDates converte datas automaticamente
    const XLSX = require('xlsx');
    const wb = XLSX.read(buf, { type: 'buffer', cellDates: true });
    const abas: string[] = wb.SheetNames || [];
    const nomeAba = opts?.aba && abas.includes(opts.aba) ? opts.aba : abas[0];
    const ws = wb.Sheets[nomeAba];
    if (!ws) return { linhas: [], aba: '', abas };
    const mat: any[][] = XLSX.utils.sheet_to_json(ws, { header: 1, raw: false, dateNF: 'yyyy-mm-dd', defval: '', blankrows: false });
    const linhas: string[][] = mat.slice(0, maxRows).map((r) => r.map((c) => (c == null ? '' : String(c)).trim()));
    return { linhas, aba: nomeAba, abas };
  }

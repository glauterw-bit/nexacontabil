...
export function PageHeader({ icon, title, subtitle, action }: { icon?: ReactNode; title: string; subtitle?: string; action?: ReactNode }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
      <div>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: COLORS.strong, display: 'flex', alignItems: 'center', gap: 10, margin: 0, letterSpacing: '-0.01em' }}>
          {icon}{title}
        </h1>
...
export function Card({ children, accent, style, onClick }: { children: ReactNode; accent?: string; style?: any; onClick?: () => void }) {
  return (
    <div onClick={onClick} style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderLeft: accent ? `3px solid ${accent}` : `1px solid ${COLORS.border}`, borderRadius: 12, padding: 16, boxShadow: 'var(--shadow-card)', ...style }}>
      {children}
    </div>
  );
}

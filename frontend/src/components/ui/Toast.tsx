...
function ToastItem({ toast, onClose }: { toast: Toast; onClose: () => void }) {
  ...
  const colorClass =
    toast.variant === 'success' ? 'text-emerald-400 border-emerald-500/40' :
    toast.variant === 'error' ? 'text-red-400 border-red-500/40' :
    toast.variant === 'warning' ? 'text-amber-400 border-amber-500/40' :
    'text-indigo-400 border-indigo-500/40';

  return (
    <div
      className={`pointer-events-auto flex gap-3 items-start min-w-[280px] max-w-md bg-[#161b2e] border ${colorClass} rounded-lg p-3 shadow-2xl transition-all duration-150 ${
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-gray-300 break-words">{toast.message}</p>
      </div>

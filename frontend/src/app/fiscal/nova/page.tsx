    ...
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[60vh] gap-6">
        ...
        <div className="text-center">
          <h2 className="text-2xl font-bold text-white mb-2">Nota Fiscal Emitida!</h2>
          <p className="text-gray-400">A nota foi transmitida à SEFAZ e autorizada com sucesso.</p>
        </div>
        <div className="flex gap-3">
          <Link href="/fiscal" className="btn-ghost border border-[#1e2740]">← Voltar para Notas</Link>
    ...
    <div className="p-8 space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Emitir Nova Nota Fiscal</h1>
      ...
      <div className="card-aura space-y-4">
        <h2 className="text-base font-semibold text-white">1. Tipo de Nota</h2>
        <div className="flex gap-3">
          {(['NF-e', 'NFS-e', 'NF-CE'] as NFTipo[]).map(t => (
            <button key={t} onClick={() => setTipo(t)}
              className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${tipo === t ? 'bg-indigo-600 text-white border-indigo-600' : 'text-gray-400 border-[#1e2740] hover:border-indigo-500/50 hover:text-white'}`}>
              {t}
            </button>
          ))}
        </div>
        <div>
          <label className="block text-sm text-gray-400 mb-1.5">Natureza da Operação</label>
      ...
      <div className="card-aura space-y-4">
        <h2 className="text-base font-semibold text-white">2. Destinatário</h2>
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-sm text-gray-400 mb-1.5">Nome / Razão Social *</label>
          ...
          <div>
            ...
            <input type="email" value={destinatario.email} onChange={e => setDestinatario(d => ({ ...d, email: e.target.value }))}
              className="w-full bg-[#0f1117] border border-[#1e2740] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-indigo-500" placeholder="contato@empresa.com.br" />
          </div>
          <div className="col-span-2">
            <label className="block text-sm text-gray-400 mb-1.5">Logradouro</label>
            <input value={destinatario.logradouro} onChange={e => setDestinatario(d => ({ ...d, logradouro: e.target.value }))}
              className="w-full bg-[#0f1117] border border-[#1e2740] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-indigo-500" placeholder="Rua das Flores, 123" />
          </div>
          <div>
            ...
            <input value={destinatario.municipio} onChange={e => setDestinatario(d => ({ ...d, municipio: e.target.value }))}
              className="w-full bg-[#0f1117] border border-[#1e2740] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-indigo-500" placeholder="São Paulo" />
          </div>
          <div>
      ...
      <div className="card-aura space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-white">3. Itens / Serviços</h2>
          <button onClick={addItem} className="btn-ghost text-sm text-indigo-400">
            <Plus className="h-4 w-4" /> Adicionar Item
          </button>
        </div>
        <div className="space-y-3">
          {itens.map((item, idx) => (
            <div key={item.id} className="grid grid-cols-12 gap-2 items-end p-3 bg-[#0f1117] rounded-lg border border-[#1e2740]">
              <div className="col-span-5">
                {idx === 0 && <label className="block text-xs text-gray-500 mb-1">Descrição</label>}
                <input value={item.descricao} onChange={e => updateItem(item.id, 'descricao', e.target.value)}
                  className="w-full bg-[#161b2e] border border-[#1e2740] rounded px-2 py-1.5 text-white text-sm outline-none focus:border-indigo-500" placeholder="Serviço de consultoria" />
              </div>
              <div className="col-span-1">
                {idx === 0 && <label className="block text-xs text-gray-500 mb-1">Qtd</label>}
                <input type="number" min="1" value={item.quantidade} onChange={e => updateItem(item.id, 'quantidade', parseFloat(e.target.value) || 1)}
                  className="w-full bg-[#161b2e] border border-[#1e2740] rounded px-2 py-1.5 text-white text-sm outline-none focus:border-indigo-500" />
              </div>
              <div className="col-span-1">
                {idx === 0 && <label className="block text-xs text-gray-500 mb-1">Un.</label>}
                <input value={item.unidade} onChange={e => updateItem(item.id, 'unidade', e.target.value)}
                  className="w-full bg-[#161b2e] border border-[#1e2740] rounded px-2 py-1.5 text-white text-sm outline-none focus:border-indigo-500" />
              </div>
              <div className="col-span-2">
                {idx === 0 && <label className="block text-xs text-gray-500 mb-1">Vl. Unitário</label>}
              ...
              <div className="col-span-2">
                {idx === 0 && <label className="block text-xs text-gray-500 mb-1">NCM/CFOP</label>}
                <input value={item.ncmCfop} onChange={e => updateItem(item.id, 'ncmCfop', e.target.value)}
                  className="w-full bg-[#161b2e] border border-[#1e2740] rounded px-2 py-1.5 text-white text-sm outline-none focus:border-indigo-500" placeholder="5101" />
              </div>
              <div className="col-span-1 flex justify-center">
                <button onClick={() => removeItem(item.id)} disabled={itens.length === 1} className="btn-ghost p-1.5 text-red-400 hover:text-red-300 disabled:opacity-30">
        ...
        <div className="card-aura space-y-3">
          ...
          {[
            ...
            <div key={t.label} className="flex justify-between text-sm">
              ...
              <span className="text-white font-mono">{t.valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
            </div>
          ))}
          <div className="flex justify-between text-sm pt-2 border-t border-[#1e2740]">
            <span className="text-gray-400">Total Impostos</span>
            <span className="text-yellow-400 font-mono font-semibold">{totalImpostos.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
          </div>
        </div>

        <div className="card-aura space-y-3">
          <h2 className="text-base font-semibold text-white">5. Resumo da Nota</h2>
          <div className="flex justify-between text-sm"><span className="text-gray-400">Subtotal</span><span className="text-white font-mono">{subtotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div>
          <div className="flex justify-between text-sm"><span className="text-gray-400">Impostos</span><span className="text-yellow-400 font-mono">+{totalImpostos.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div>
          <div className="flex justify-between text-lg font-bold pt-2 border-t border-[#1e2740]">
            <span className="text-white">TOTAL</span>
            <span className="text-indigo-400 font-mono">{total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span>
          </div>

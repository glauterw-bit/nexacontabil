  ...
  if (!selectedCompany) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 p-8">
        ...
        <p className="text-tx-muted text-sm">Selecione uma empresa para o benchmark setorial.</p>
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-tx-strong">Benchmark Setorial</h1>
          <p className="text-tx-muted text-sm mt-1">{selectedCompany.name} · Comparativo com o setor (últimos 12 meses)</p>
        </div>
        <div className="flex items-center gap-3">
          <Target className="h-4 w-4 text-acao" />
          <select
            value={setor}
        ...
        <>
          <div className="flex items-center gap-3 bg-indigo-600/10 border border-indigo-500/30 rounded-xl p-4">
            <div className="h-12 w-12 rounded-xl bg-indigo-600/20 flex items-center justify-center flex-shrink-0">
              <span className="text-acao font-bold text-lg">{data.acimaDaMedia}/{data.totalMetricasComputaveis}</span>
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {computaveis.length > 0 && (
              <div className="card-aura">
                <h2 className="text-base font-semibold text-tx-strong mb-4">Mapa de Competitividade</h2>
            ...
            <div className="card-aura">
              <h2 className="text-base font-semibold text-tx-strong mb-4">Comparativo por Indicador</h2>
              <div className="space-y-3">
                {metricas.map(m => {
                  ...
                  return (
                    <div key={m.dimensao} className={`p-3 bg-inset rounded-lg border ${isComputed ? 'border-line' : 'border-line opacity-60'}`}>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {isComputed ? (
                          <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${isMelhor ? 'text-ok bg-green-400/10 border-green-400/30' : 'text-err bg-red-400/10 border-red-400/30'}`}>
                            {isMelhor ? 'Acima da média' : 'Abaixo da média'}
                          </span>
                        ) : (
                          <span className="text-xs px-2 py-0.5 rounded-full border border-gray-500/30 bg-gray-500/10 text-tx-muted">N/D</span>
                        )}
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-sm">
                        <div className="text-center">
                          <p className="text-xs text-tx-muted mb-0.5">Empresa</p>
                          <p className={`font-bold font-mono ${isComputed ? (isMelhor ? 'text-ok' : 'text-err') : 'text-tx-faint'}`}>
                            {isComputed ? `${m.empresa}${m.unidade}` : '—'}
                          </p>
                        </div>
                        <div className="text-center">
                          <p className="text-xs text-tx-muted mb-0.5">Setor</p>
                          <p className="font-medium font-mono text-tx">{m.setor}{m.unidade}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-xs text-tx-muted mb-0.5">Diferença</p>
                          <p className={`font-medium text-sm flex items-center justify-center gap-1 ${isComputed ? (isMelhor ? 'text-ok' : 'text-err') : 'text-tx-faint'}`}>
                            {isComputed ? (
                              <>

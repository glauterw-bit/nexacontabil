...
const statusConfig: Record<string, { label: string; icon: any; color: string }> = {
  draft:          { label: 'Rascunho',    icon: Clock,         color: 'text-tx-muted' },
  pending_review: { label: 'Aguardando',  icon: Clock,         color: 'text-warn' },
  approved:       { label: 'Aprovado',    icon: CheckCircle,   color: 'text-ok' },
  rejected:       { label: 'Rejeitado',   icon: XCircle,       color: 'text-err' },
  reconciled:     { label: 'Reconciliado', icon: CheckCircle,  color: 'text-info' },
};
  ...
  if (!selectedCompany) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 p-8">
        ...
        <p className="text-tx-muted text-sm">Selecione uma empresa para ver os lançamentos.</p>
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-tx-strong">Lançamentos Contábeis</h1>
          <p className="text-tx-muted text-sm mt-1">{selectedCompany.name}</p>
        </div>
      ...
      <div className="card-aura">
        {loading ? (
          <div className="text-center py-12 text-tx-muted">Carregando lançamentos...</div>
        ) : transactions.length === 0 ? (
          <div className="text-center py-12">
            <TrendingUp className="h-12 w-12 text-tx-faint mx-auto mb-3" />
            <p className="text-tx-muted text-sm">Nenhum lançamento encontrado.</p>
            <p className="text-tx-faint text-xs mt-1">Envie um documento para gerar lançamentos automáticos.</p>
          </div>
        ) : (
          <table className="w-full">
            ...
            <tbody className="divide-y divide-line">
              {transactions.map((tx: any) => {
                ...
                const Icon = cfg.icon;
                return (
                  <tr key={tx.id} className="hover:bg-inset transition-colors">
                    <td className="py-3 text-sm text-tx-muted">
                      {new Date(tx.date).toLocaleDateString('pt-BR')}
                    </td>
                    <td className="py-3 text-sm text-tx-strong max-w-xs truncate">{tx.description}</td>
                    <td className="py-3 text-sm text-right font-mono">
                      <span className="flex items-center justify-end gap-1 text-err">
                        <TrendingDown className="h-3 w-3" />
                        {Number(tx.totalDebit).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                      </span>
                    </td>
                    <td className="py-3 text-sm text-right font-mono">
                      <span className="flex items-center justify-end gap-1 text-ok">
                    ...
                    <td className="py-3 text-center">
                      {tx.aiConfidence ? (
                        <span className="text-xs text-acao font-mono">

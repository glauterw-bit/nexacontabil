  ...
  return (
    <div className="flex items-start justify-between gap-6">
      <div className="flex-1">
        ...
        {desc && <p className="text-xs text-tx-muted mt-0.5">{desc}</p>}
      </div>
            ...
            <>
              <Section title="Informações do Perfil">
                <form onSubmit={handleSaveProfile} className="space-y-4">
                  <div>
                    <label className="block text-xs text-tx-muted mb-1.5">Nome</label>
                    <input
                      value={name}
                  ...
                  <div>
                    <label className="block text-xs text-tx-muted mb-1.5">E-mail</label>
                    <input
                      value={user?.email ?? ''}
                      disabled
                  ...
                  <div>
                    <label className="block text-xs text-tx-muted mb-1.5">Função</label>
                    <input
                      value={user?.role ?? ''}
                      disabled
                ...
                <Section title="Empresa Selecionada">
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-tx-muted">Nome</span>
                      <span className="text-tx">{selectedCompany.name}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-tx-muted">CNPJ</span>
                      <span className="text-tx font-mono">
                        {selectedCompany.cnpj.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5')}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-tx-muted">Regime</span>
                      <span className="text-tx">{selectedCompany.taxRegime?.replace('_', ' ')}</span>
                    </div>
                  </div>
                </Section>
              )}
            ...
            <Section title="Alterar Senha">
              <form onSubmit={handleSavePassword} className="space-y-4">
                <div className="relative">
                  <label className="block text-xs text-tx-muted mb-1.5">Nova senha</label>
                  <div className="relative">
                    <input
                ...
                <div>
                  <label className="block text-xs text-tx-muted mb-1.5">Confirmar nova senha</label>
                  <input
                    type={showPass ? 'text' : 'password'}
              ...
              <Section title="Modelo e Parâmetros">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs text-tx-muted mb-1.5">Temperature ({iaSettings.temperature})</label>
